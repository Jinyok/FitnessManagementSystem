<template>
    <div>
            <!--div class="card" style="flex-grow: 1; margin-right: 16px;">
                <div style="cursor: pointer; display: flex; align-item: center;" @click="addCoach()">
                    ...
                    <p class="list-text" style="margin: 0px 8px">增添教练</p>
                </div>
            </div-->
        <div class="card">
        ...
        <div v-for="coach in coachList" :key="coach.id">
            ...
            >
                <div>
                    <h2 class="list-title">{{ coach.name }}</h2>
                    <p class="info">id: {{ coach.id }}</p>
                </div>
        ...
        search(type, content) {
            if (type == 'id') {
                ...
                for (var coach of this.coaches)
                    ...
                    {
                        flag = false;
                        this.$router.push({ name: 'ManagerCoachDetails', params: { id: coach.id }});
                    }
                if (flag)
                ...
                var list = [];
                for (var coach of this.coaches)
                    if (coach.name.includes(content))
                        list.push(coach);
                this.coachList = list;
            }
        },
        jump(id) {
            this.$router.push({ name: 'ManagerCoachDetails', params: { id: id }});
        }
    }
}
</script>
